...
const buttonVariants = {
  ...sectionVariants,
  tap: { scale: 0.98, boxShadow: "0 4px 10px rgba(142, 45, 226, 0.2)" },
};

export default function ContactSection() {

  // ✅ Replace this with your actual Zoho Bookings URL
  const zohoBookingLink = "YOUR_ZOHO_BOOKINGS_LINK_HERE";

  const handleSchedule = () => {
    if (zohoBookingLink === "YOUR_ZOHO_BOOKINGS_LINK_HERE") {
      console.error("Zoho Bookings link not set.");
      ...
      <div className="relative z-10 max-w-4xl">
        ...
        <motion.button
          ...
          className="flex items-center justify-center gap-3 mx-auto px-10 py-4 
                     ...
                     text-xl rounded-full shadow-xl transition-all duration-300 
                     hover:shadow-2xl hover:shadow-[#8e2de2]/50 
                     focus:outline-none focus:ring-4 focus:ring-[#3b82f6]/50"
        >
          <IconCalendar className="w-6 h-6 stroke-2" />
          Book via Zoho Bookings
        </motion.button>
      </div>
    </section>
  );
